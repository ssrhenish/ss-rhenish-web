...
export default function AnniversaryPage() {
  ...
  const memberStaff = ANNIVERSARY_ISSUES.filter((item) => item.category === "教會會友");

  const tabsList = ["趙氏訪談", "蘇家三代專輯", "陳氏專訪", "李家專訪", "學生團契"];
    ...
    <div className="bg-[#faf7f2] min-h-screen font-sans text-slate-800 antialiased">
      ...
      <header className="bg-gradient-to-b from-[#8c2525] to-[#591414] text-white py-24 px-4 text-center border-b-8 border-yellow-600 shadow-xl">
        <div className="max-w-3xl mx-auto">
          <div className="w-16 h-16 bg-yellow-500/10 border border-yellow-500 rounded-full mx-auto flex items-center justify-center text-2xl mb-4">
            👑
          </div>
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-yellow-500 mb-4">
            五十週年紀念特刊
          </h1>
          <p className="text-xs tracking-widest text-yellow-200/80 uppercase font-semibold mb-6">
            金禧感恩多媒體專頁
          </p>
          <button
            onClick={() => scrollToSection("interviews")}
            className="bg-yellow-600 hover:bg-yellow-500 text-white text-xs font-bold px-6 py-2.5 rounded-lg shadow transition-colors cursor-pointer"
          >
            🎬 前往影音專訪與紀錄片
          </button>
        </div>
      ...
      <nav className="max-w-6xl mx-auto px-4 -mt-10 relative z-10">
        <div className="bg-white border border-yellow-600/20 rounded-2xl shadow-xl p-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {NAVIGATION_ANCHORS.map((anchor) => (
            <button
              key={anchor.id}
              onClick={() => scrollToSection(anchor.id)}
              className="flex items-center gap-2.5 p-3 rounded-xl bg-[#fffdf9] border border-slate-100 hover:border-yellow-600/40 hover:bg-yellow-50/20 text-left transition-all shadow-sm cursor-pointer"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 py-16 space-y-16">
        ...
        <section
          id="pastoral"
          className="scroll-mt-12 bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200/60"
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {pastoralStaff.map((staff) => {
              ...
              return (
                <div
                  key={staff.id}
                  className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-center flex flex-col justify-between items-center group"
                >
                  ...
                  {videoId ? (
                    <button
                      onClick={() => setActiveVideo(staff)}
                      className="w-full bg-[#8c2525] hover:bg-[#731d1d] text-white text-[10px] font-bold py-1.5 rounded-lg transition-colors cursor-pointer"
                    >
                      🎬 觀看感恩影音
                    </button>
                  ) : (
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {fellowships.map((fellow) => (
              <div
                key={fellow.id}
                className="border border-slate-200/80 rounded-xl p-4 bg-[#fffdfa] flex flex-col justify-between hover:border-emerald-600/40 transition-colors"
              >
                ...
                {fellow.videoId ? (
                  <button
                    ...
                    className="text-[10px] bg-emerald-50 text-emerald-700 font-bold px-2 py-1 rounded text-center mt-2 hover:bg-emerald-100 transition-colors cursor-pointer"
                  >
                    🎬 播放回顧短片
                  </button>
                ) : (
                  <span className="text-[9px] text-slate-400 mt-2 block italic">
                    精彩圖集整理中
                  </span>
                )}
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent z-10 flex flex-col justify-end p-4">
                    <span className="text-[9px] bg-yellow-600 text-white px-1.5 py-0.5 rounded w-max font-bold mb-1 uppercase">
                      {video.subSection}
                    </span>
                    <h4 className="text-white font-bold text-xs line-clamp-1 group-hover:text-yellow-400 transition-colors">
                      {video.title}
                    </h4>
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center z-10">
                    <div className="bg-red-600 text-white w-9 h-9 rounded-full flex items-center justify-center text-xs shadow-lg transform group-hover:scale-110 transition-all">
                      ▶
                    </div>
                  </div>
                </div>
        ...
        >
          <div className="border-l-4 border-amber-600 pl-4 mb-8">
            <h2 className="text-lg font-bold text-slate-900">教會會友</h2>
            <p className="text-xs text-slate-400 mt-1">分享與見證</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-[20px] lg:gap-[48px]">
            {memberStaff.map((member) => {
              const imageSize = member.imageSize || 64;
              const isRounded = member.rounded ?? true;
              return (
                <div
                  key={member.id}
                  className="flex flex-col items-center justify-start text-center group cursor-pointer"
                  onClick={() => member.videoId && setActiveVideo(member)}
                >
                  <div className="overflow-hidden flex justify-center items-center">
                    <img
                      src={member.imageSrc}
                      alt={member.author || member.title}
                      width={imageSize}
                      height={imageSize}
                      loading="lazy"
                  ...
                  <div className="mt-3">
                    <p className="text-[18px] md:text-[20px] font-medium leading-[1.5] text-[#333] group-hover:underline">
                      {member.author || member.title}
                    </p>
                  </div>
                </div>
              );
        ...
        {/* ─── 珍貴時刻 (Precious Moments Gallery) ─── */}
        <section
          id="precious-moments"
          className="scroll-mt-12 bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200/60 overflow-hidden"
        >
          <div className="border-l-4 border-amber-600 pl-4 mb-8">
            <h2 className="text-lg font-bold text-slate-900">珍貴時刻</h2>
            <p className="text-xs text-slate-400 mt-1">典藏影集歷史影像紀錄</p>
          </div>

          {/* Flex layout replicating Gutenberg block structure */}
          <div className="flex flex-col sm:flex-row flex-wrap lg:flex-nowrap items-center justify-center gap-6 lg:gap-[24px] pt-4 pb-4">
            {PRECIOUS_MOMENTS_IMAGES.map((img) => (
              <img
                key={img.id}
                src={img.src}
                alt={img.alt}
                loading="lazy"
                className={`w-full sm:w-[calc(50%-12px)] lg:w-[282px] object-cover rounded-[6px] shadow-sm transition-transform duration-300 hover:scale-[1.02] ${img.heightClass} ${img.marginClass}`}
              />
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc) => (
              <a
                ...
                className="block p-5 bg-[#fafbfc] border border-slate-200 rounded-xl hover:border-blue-600 hover:bg-blue-50/10 transition-all group"
              >
                <div className="flex items-start justify-between">
                  <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600">
                    📄
                  </div>
                  <span className="text-[10px] text-blue-600 font-bold border border-blue-200 rounded px-1.5 py-0.5 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                    打開文獻 ↗
                  </span>
                </div>
                <h4 className="font-bold text-xs text-slate-900 mt-4 group-hover:text-blue-700 transition-colors">
                  {doc.title}
                </h4>
      ...
      {activeVideo && (
        ...
        >
          <div
            className="bg-slate-900 rounded-2xl max-w-3xl w-full overflow-hidden shadow-2xl relative border border-slate-800 animate-in fade-in zoom-in-95 duration-150 cursor-default"
            onClick={(e) => e.stopPropagation()}
          >
            {(() => {
              const videoId = cleanYouTubeId(activeVideo.videoId);
              if (videoId) {
                const embedUrl = `https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&rel=0&enablejsapi=1${
            ...
            <div className="p-4 bg-slate-950 flex items-center justify-between text-white">
              <div className="text-left">
                <span className="text-[9px] font-bold text-yellow-500 block uppercase">
                  ...
                  {activeVideo.subSection ? `• ${activeVideo.subSection}` : ""}
                </span>
